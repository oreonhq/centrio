...
class ProgressPage(Gtk.Box):
    ...
    def _copy_live_environment(self, config_data):
        ...
        has_saved_network_config = bool(network_config)
        ...
        if skip_network:
            ...
            print("Network configuration explicitly skipped by user; additional software installation is disabled.")
            return True

        # If network page wasn't explicitly applied, detect current connectivity from runtime state.
        if not has_saved_network_config:
            detected = backend.check_network_connectivity()
            network_enabled = detected
            print(f"No saved network config; runtime connectivity detected: {detected}")

        # For selected extras, never silently skip due transient network state.
        if has_extra_work:
            # Refresh DNS/network state before DNF/Flatpak operations.
            self._update_progress_text("Refreshing network...", 0.68)
            backend.restart_network_manager()
            self._update_progress_text("Checking network connectivity...", 0.68)

            # Retry briefly because Wi-Fi association and DNS can settle moments later.
            ...
            if not network_enabled:
                ...
                self._update_progress_text(self.installation_error, 0.70)
    ...
    def _remove_live_users_and_configure_oobe(self, config_data):
        ...
        User creation is handled by OOBE (gnome-initial-setup) on first login."""
